...
export default function SlugReservationPage() {
  ...
  useEffect(() => {
    fetch(`/api/restaurant?slug=${slug}`)
      .then(r => r.json())
      .then(data => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) setSucces({ table: data.table })
    else setErreur(data.error || 'Une erreur est survenue, veuillez réessayer.')
    setLoading(false)
  }
  ...
  const wrap = (content: React.ReactNode) => (
    <div style={{ minHeight: '100vh', background: '#faf9f7', fontFamily: 'Georgia, serif', padding: '40px 24px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      {content}
    </div>
  )

  if (!restaurant && !notFound) return wrap(
    <p style={{ color: '#888', fontSize: 14 }}>Chargement…</p>
  )

  if (notFound) return wrap(
    <div style={{ textAlign: 'center' }}>
      <h2 style={{ fontSize: 22, fontWeight: 'normal', color: '#1a1a1a', marginBottom: 12 }}>Restaurant introuvable</h2>
      <p style={{ color: '#888', fontSize: 14 }}>Ce lien de réservation est invalide.</p>
    </div>
  )

  if (succes) return wrap(
    <div style={{ background: 'white', padding: '48px', borderRadius: '4px', textAlign: 'center', maxWidth: '480px', width: '100%', border: '1px solid #e8e4de' }}>
      <div style={{ width: '56px', height: '56px', borderRadius: '50%', background: '#f0f7f0', border: '1px solid #c3dfc3', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px', fontSize: '24px' }}>✓</div>
      <h2 style={{ fontSize: '24px', fontWeight: 'normal', margin: '0 0 8px', color: '#1a1a1a' }}>Réservation confirmée</h2>
      ...
      <button
        ...
        style={{ background: 'none', border: '1px solid #c9a96e', color: '#c9a96e', padding: '10px 24px', borderRadius: '2px', cursor: 'pointer', fontSize: '13px', letterSpacing: '0.08em', textTransform: 'uppercase' }}
      >
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            <div>
              ...
              <input name="nom" required value={form.nom} onChange={handleChange} style={inputStyle} placeholder="Jean Dupont" />
            </div>
            <div>
              <label style={labelStyle}>Téléphone *</label>
              <input name="telephone" required value={form.telephone} onChange={handleChange} style={inputStyle} placeholder="06 12 34 56 78" />
            </div>
          </div>

          <div>
            <label style={labelStyle}>Email *</label>
            <input name="email" type="email" required value={form.email} onChange={handleChange} style={inputStyle} placeholder="jean@email.com" />
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
            ...
            <div>
              ...
              <input name="date" type="date" required min={today} value={form.date} onChange={handleChange} style={inputStyle} />
            </div>
            <div>
          ...
          {erreur && (
            <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '2px', padding: '12px 14px', fontSize: '13px', color: '#dc2626' }}>
              {erreur}
            </div>
          )}
